...
the Coro module itself nor any of it's submodules will ever give up the
...
point of execution.  Not os obviously, it also has to include all
...
it's own per-thread C<$string> variable, it would probably print
C<World!\nWorld\n>, which is rather unexpected, and would make it very
difficult to make good use of threads.

There are quite a numbe rof other things that are per-thread:
